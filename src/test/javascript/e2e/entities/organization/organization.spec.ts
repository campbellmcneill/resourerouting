...
describe('Organization e2e test', () => {
  ...
  it('should load Organizations', async () => {
    ...
    expect(await organizationComponentsPage.getTitle()).to.eq('resourceroutingApp.organization.home.title');
    await browser.wait(
  ...
  it('should load create Organization page', async () => {
    ...
    expect(await organizationUpdatePage.getPageTitle()).to.eq('resourceroutingApp.organization.home.createOrEditLabel');
  ...
  it('should create and save Organizations', async () => {
    ...
    await promise.all([
      organizationUpdatePage.setOrganizationNameInput('organizationName'),
      organizationUpdatePage.locationSelectLastOption(),
      organizationUpdatePage.opportunitySelectLastOption()
    ]);

    expect(await organizationUpdatePage.getOrganizationNameInput()).to.eq(
      'organizationName',
      'Expected OrganizationName value to be equals to organizationName'
    );
  ...
  it('should delete last Organization', async () => {
    ...
    expect(await organizationDeleteDialog.getDialogTitle()).to.eq('resourceroutingApp.organization.delete.question');
